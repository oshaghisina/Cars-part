...
export const useLeadsStore = defineStore('leads', {
  ...
  getters: {
    ...
    filteredLeads: (state) => {
      ...
      if (state.filters.dateTo) {
        filtered = filtered.filter(lead => 
          new Date(lead.created_at) <= new Date(state.filters.dateTo)
        )
      }
    ...
    leadsByStatus: (state) => {
      ...
      state.leads.forEach(lead => {
        ...
        } else if (new Date(lead.created_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)) {
    ...
    leadsWithOrders: (state) => {
      return state.leads.filter(lead => lead.orders && lead.orders.length > 0)
    },
  ...
  actions: {
    async fetchLeads(params = {}) {
      ...
      try {
        ...
        this.pagination = {
          ...
          totalPages: Math.ceil((response.headers['x-total-count'] || response.data.length) / (params.limit || 20))
        }
    ...
    async fetchLeadOrders(leadId = null) {
      try {
        if (leadId) {
          ...
          const response = await axios.get(`${API_BASE}/orders/?lead_id=${leadId}`)
    ...
    calculateStats() {
      ...
      const totalOrders = this.leads.reduce((total, lead) => total + (lead.orders?.length || 0), 0)
    ...
    getLeadStatus(lead) {
      if (lead.orders && lead.orders.length > 0) {
        ...
        return 'new'
      } else if (new Date(lead.created_at) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)) {
        return 'active'
      } else {
        return 'inactive'
      }
