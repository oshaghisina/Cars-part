  ...
  actions: {
    ...
    async updateOrderItem(orderId, lineNo, itemData) {
      ...
      try {
        ...
        this.error = error.response?.data?.detail || 'Failed to update order item'
    ...
    async removeOrderItem(orderId, lineNo) {
      ...
      try {
        ...
        this.error = error.response?.data?.detail || 'Failed to remove order item'
    ...
    async getOrderCustomer(orderId) {
      try {
        const order = await this.getOrder(orderId)
        if (order.lead_id) {
          const response = await axios.get(`${API_BASE}/leads/${order.lead_id}`)
          return response.data
        }
        return null
      } catch (error) {
    ...
    async getOrdersByCustomer(customerId) {
      try {
        const response = await axios.get(`${API_BASE}/orders/?lead_id=${customerId}`)
        return response.data
      } catch (error) {
