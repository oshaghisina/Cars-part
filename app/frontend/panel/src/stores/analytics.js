...
export const useAnalyticsStore = defineStore('analytics', {
  ...
  getters: {
    ...
    getLowStockCount: (state) => state.inventoryAnalytics.low_stock_items.length,
  ...
  actions: {
    ...
    async fetchDashboardMetrics(dateFrom = null, dateTo = null) {
      ...
      try {
        ...
        const response = await axios.get(`/api/v1/analytics/dashboard/metrics?${params}`)
        this.dashboardMetrics = response.data
      } catch (error) {
        this.error = error.response?.data?.detail || 'Failed to fetch dashboard metrics'
    ...
    async fetchSalesAnalytics(period = '30d') {
      ...
      try {
        const response = await axios.get(`/api/v1/analytics/sales/analytics?period=${period}`)
        this.salesAnalytics = response.data
      } catch (error) {
        this.error = error.response?.data?.detail || 'Failed to fetch sales analytics'
    ...
    async fetchInventoryAnalytics() {
      ...
      try {
        const response = await axios.get('/api/v1/analytics/inventory/analytics')
        this.inventoryAnalytics = response.data
      } catch (error) {
        this.error = error.response?.data?.detail || 'Failed to fetch inventory analytics'
    ...
    async fetchCustomerAnalytics(period = '30d') {
      ...
      try {
        const response = await axios.get(`/api/v1/analytics/customers/analytics?period=${period}`)
        this.customerAnalytics = response.data
      } catch (error) {
        this.error = error.response?.data?.detail || 'Failed to fetch customer analytics'
    ...
    async fetchPerformanceMetrics() {
      ...
      try {
        const response = await axios.get('/api/v1/analytics/performance/metrics')
    ...
    async fetchPartsByCategoryChart() {
      ...
      try {
        const response = await axios.get('/api/v1/analytics/charts/parts-by-category')
        this.chartsData.partsByCategory = response.data
      } catch (error) {
        this.error = error.response?.data?.detail || 'Failed to fetch parts by category chart'
    ...
    async fetchSalesTrendChart(period = '30d') {
      ...
      try {
        const response = await axios.get(`/api/v1/analytics/charts/sales-trend?period=${period}`)
        this.chartsData.salesTrend = response.data
      } catch (error) {
        this.error = error.response?.data?.detail || 'Failed to fetch sales trend chart'
    ...
    async generateReport(reportRequest) {
      ...
      try {
        const response = await axios.post('/api/v1/analytics/reports/generate', reportRequest)
        this.currentReport = response.data
        return response.data
      } catch (error) {
        this.error = error.response?.data?.detail || 'Failed to generate report'
    ...
    async exportData(exportRequest) {
      ...
      try {
        const response = await axios.post('/api/v1/analytics/export', exportRequest, {
          responseType: 'blob'
        })
