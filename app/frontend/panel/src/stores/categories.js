  ...
  getters: {
    ...
    getCategoryPath: (state) => (categoryId) => {
      ...
      while (current) {
        ...
        current = current.parent_id ? state.categories.find(cat => cat.id === current.parent_id) : null
      }
    ...
    getDescendants: (state) => (categoryId) => {
      const descendants = []
      const children = state.categories.filter(cat => cat.parent_id === categoryId)
  ...
  actions: {
    ...
    async fetchCategoryTree() {
      ...
      try {
        ...
        this.error = error.response?.data?.detail || 'Failed to fetch category tree'
    ...
    async updateCategory(id, categoryData) {
      ...
      try {
        ...
        this.error = error.response?.data?.detail || 'Failed to update category'
    ...
    async getCategoryChildren(id) {
      ...
      this.error = null
      try {
        const response = await axios.get(`${API_BASE}/categories/${id}/children`)
        return response.data
      } catch (error) {
        this.error = error.response?.data?.detail || 'Failed to fetch category children'
    ...
    async getCategoryPath(id) {
      ...
      try {
        ...
        this.error = error.response?.data?.detail || 'Failed to fetch category path'
    ...
    async searchCategories(query) {
      ...
      this.error = null
      try {
        const response = await axios.get(`${API_BASE}/categories/search?q=${encodeURIComponent(query)}`)
        return response.data
      } catch (error) {
        this.error = error.response?.data?.detail || 'Failed to search categories'
