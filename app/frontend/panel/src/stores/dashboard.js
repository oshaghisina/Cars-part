...
export const useDashboardStore = defineStore('dashboard', {
  ...
  getters: {
    // Computed metrics
    totalSystems: (state) => state.metrics.totalParts + state.metrics.totalVehicles + state.metrics.totalCategories,
    ...
    overallHealth: (state) => {
      ...
      return statuses.every(status => status === 'healthy') ? 'healthy' : 'warning'
    },
  ...
  actions: {
    ...
    async fetchMetrics() {
      try {
        ...
        this.metrics.activeParts = parts.filter(part => part.status === 'active').length
        
        // Fetch vehicles metrics
        const [brandsResponse, modelsResponse, trimsResponse] = await Promise.all([
          axios.get(`${API_BASE}/vehicles/brands`),
          axios.get(`${API_BASE}/vehicles/models`),
          axios.get(`${API_BASE}/vehicles/trims`)
        ])
        this.metrics.totalVehicles = brandsResponse.data.length + modelsResponse.data.length + trimsResponse.data.length
        ...
        this.metrics.pendingOrders = orders.filter(order => ['new', 'in_progress', 'quoted'].includes(order.status)).length
        ...
        try {
          ...
          this.metrics.newLeads = leads.filter(lead => new Date(lead.created_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length
        } catch (error) {
    ...
    async fetchChartsData() {
      try {
        ...
        parts.forEach(part => {
          ...
          if (part.category) {
            categoryCount[part.category] = (categoryCount[part.category] || 0) + 1
          }
          
          // Brand distribution
          if (part.vehicle_make) {
            brandCount[part.vehicle_make] = (brandCount[part.vehicle_make] || 0) + 1
          }
        })
        ...
        this.charts.orderStatusDistribution = Object.entries(orderStatusCount)
          .map(([name, value]) => ({ name, value }))
        
        // Lead status distribution (mock data for now)
        this.charts.leadStatusDistribution = [
          { name: 'new', value: this.metrics.newLeads },
          { name: 'contacted', value: Math.floor(this.metrics.totalLeads * 0.3) },
          { name: 'qualified', value: Math.floor(this.metrics.totalLeads * 0.2) },
    ...
    async fetchRecentActivity() {
      try {
        ...
        const categoriesResponse = await axios.get(`${API_BASE}/categories/?limit=5`)
    ...
    async checkSystemHealth() {
      try {
        ...
        this.systemHealth.apiStatus = responseTime < 1000 ? 'healthy' : 'warning'
    ...
    getFormattedMetric(metric, value) {
      switch (metric) {
        ...
        case 'pendingOrders':
          return { value: value.toString(), label: 'Pending Orders', icon: '⏳' }
        case 'newLeads':
