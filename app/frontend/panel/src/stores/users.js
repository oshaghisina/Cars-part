  ...
  getters: {
    ...
    filteredUsers: (state) => {
      ...
      if (state.filters.isActive !== null) {
        filtered = filtered.filter(user => user.is_active === state.filters.isActive)
      }
  ...
  actions: {
    async fetchUsers() {
      ...
      try {
        ...
        const response = await axios.get(`${API_BASE}/users/`, { params, headers })
    ...
    async fetchUserById(id) {
      ...
      try {
        const token = localStorage.getItem('access_token')
        const headers = token ? { Authorization: `Bearer ${token}` } : {}
        const response = await axios.get(`${API_BASE}/users/${id}`, { headers })
    ...
    async createUser(userData) {
      ...
      try {
        const token = localStorage.getItem('access_token')
        const headers = token ? { Authorization: `Bearer ${token}` } : {}
        const response = await axios.post(`${API_BASE}/users/`, userData, { headers })
    ...
    async updateUser(id, userData) {
      ...
      try {
        const token = localStorage.getItem('access_token')
        const headers = token ? { Authorization: `Bearer ${token}` } : {}
        const response = await axios.put(`${API_BASE}/users/${id}`, userData, { headers })
        this.fetchUsers() // Refresh list
        return response.data
      } catch (error) {
    ...
    async fetchUserStatistics() {
      try {
        const token = localStorage.getItem('access_token')
        const headers = token ? { Authorization: `Bearer ${token}` } : {}
        const response = await axios.get(`${API_BASE}/users/statistics/overview`, { headers })
    ...
    async changePassword(passwordData) {
      ...
      try {
        const token = localStorage.getItem('access_token')
        const response = await axios.post(`${API_BASE}/users/change-password`, passwordData, {
          headers: { Authorization: `Bearer ${token}` }
        })
        return response.data
      } catch (error) {
    ...
    async assignRoleToUser(userId, roleName) {
      try {
        const token = localStorage.getItem('access_token')
        await axios.post(`${API_BASE}/users/${userId}/roles/${roleName}`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        })
        this.fetchUsers() // Refresh list
      } catch (error) {
