    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
      <div class="mt-3">
        ...
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-medium text-gray-900">Add Vehicle Model</h3>
          <button
            @click="$emit('close')"
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <!-- Model Name (English) -->
          <div>
            <label for="name" class="block text-sm font-medium text-gray-700">Model Name (English) *</label>
            <input
              id="name"
              ...
              required
              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
              placeholder="e.g., Tiggo 8"
            />
          </div>

          <!-- Model Name (Persian) -->
          <div>
            <label for="name_fa" class="block text-sm font-medium text-gray-700">Model Name (Persian)</label>
            <input
              id="name_fa"
              v-model="formData.name_fa"
              type="text"
              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
              placeholder="e.g., تیگو 8"
            />
          </div>

          <!-- Model Name (Chinese) -->
          <div>
            <label for="name_cn" class="block text-sm font-medium text-gray-700">Model Name (Chinese)</label>
            <input
              id="name_cn"
...
import { useVehicleBrandsStore, useVehicleModelsStore } from '../../stores/vehicles'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const handleSubmit = async () => {
      ...
      try {
        await modelsStore.createModel(formData)
        emit('saved')
      } catch (err) {
