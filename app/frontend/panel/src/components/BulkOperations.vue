  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center space-x-2">
        <span class="text-sm text-gray-500">{{ selectedCount }} items selected</span>
        <button
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <div class="space-y-2">
                <label class="flex items-center">
                  ...
                  >
                  <span class="ml-2 text-sm text-gray-700">Create new records only</span>
                </label>
                <label class="flex items-center">
                  <input
                    v-model="importOptions.mode"
                    ...
                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                  >
                  <span class="ml-2 text-sm text-gray-700">Update existing records</span>
                </label>
                <label class="flex items-center">
                  <input
                    v-model="importOptions.mode"
                    ...
                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                  >
                  <span class="ml-2 text-sm text-gray-700">Create or update records</span>
                </label>
              </div>
            </div>
              ...
              <div class="space-y-2">
                ...
                <label class="flex items-center">
                  <input
                    ...
                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  >
                  <span class="ml-2 text-sm text-gray-700">Skip rows with errors</span>
                </label>
              </div>
            </div>
            ...
            >
              <svg v-if="isImporting" class="w-4 h-4 mr-2 inline animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="importProgress.show" class="mt-6">
        <div class="bg-gray-50 rounded-lg p-4">
          <div class="flex items-center justify-between mb-2">
            <span class="text-sm font-medium text-gray-700">{{ importProgress.message }}</span>
            <span class="text-sm text-gray-500">{{ importProgress.percentage }}%</span>
          </div>
          ...
          <div class="mt-2 text-sm text-gray-600">
            {{ importProgress.processed }} of {{ importProgress.total }} records processed
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <div class="max-h-32 overflow-y-auto border border-gray-300 rounded-md p-2">
                <label v-for="field in availableFields" :key="field.id" class="flex items-center mb-1">
                  <input
        ...
        <div>
          ...
          <div class="space-y-4">
            ...
            >
              <svg v-if="isExporting" class="w-4 h-4 mr-2 inline animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
              <svg v-else class="w-4 h-4 mr-2 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div v-if="recentExports.length > 0">
              <h5 class="text-sm font-medium text-gray-700 mb-2">Recent Exports</h5>
              <div class="space-y-2">
                <div
                ...
                >
                  <div class="flex items-center">
                    ...
                    <div>
                      <div class="text-sm text-gray-700">{{ exportItem.name }}</div>
                      <div class="text-xs text-gray-500">{{ formatDate(exportItem.createdAt) }}</div>
                    </div>
                  </div>
                  <button
...
export default {
  ...
  emits: ['import-complete', 'export-complete', 'batch-operation'],
  setup(props, { emit }) {
    ...
    const operationTabs = [
      { id: 'import', name: 'Import', icon: 'M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M9 19l3 3m0 0l3-3m-3 3V10' },
      ...
      { id: 'batch', name: 'Batch Operations', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' }
    ]
    ...
    const confirmBatchDelete = () => {
      if (confirm(`Are you sure you want to delete ${props.selectedItems.length} items? This action cannot be undone.`)) {
        emit('batch-operation', {
          type: 'delete',
          items: props.selectedItems
        })
