  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
    <div class="flex items-center justify-between mb-4">
      ...
      >
        ...
        <svg class="w-4 h-4 ml-1 transform transition-transform" :class="{ 'rotate-180': isExpanded }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-show="isExpanded" class="space-y-4 transition-all duration-300">
      <!-- Module Selection -->
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">Search in Modules</label>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
          <label v-for="module in modules" :key="module.id" class="flex items-center">
            <input
    ...
    <div v-if="searchResults.length > 0" class="mt-4">
      <h4 class="text-sm font-medium text-gray-700 mb-2">Quick Results Preview</h4>
        ...
        >
          <div class="flex items-center justify-between">
            <div>
              ...
              <span class="ml-2 text-sm text-gray-500">{{ result.module }}</span>
            </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const selectedModules = ref(['vehicles', 'parts', 'orders', 'leads', 'users'])
    ...
    const savedSearches = ref([
      { id: 1, name: 'Active Parts', filters: { status: 'active', modules: ['parts'] } },
      { id: 2, name: 'Recent Orders', filters: { dateFrom: '2024-01-01', modules: ['orders'] } },
      { id: 3, name: 'High Value Items', filters: { priceMin: '1000', modules: ['parts'] } }
    ])

    const hasActiveFilters = computed(() => {
      return quickSearch.value.trim() !== '' || 
