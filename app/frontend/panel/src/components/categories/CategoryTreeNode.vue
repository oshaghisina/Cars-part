        ...
        <div class="flex items-center justify-between w-full">
          <div class="flex items-center space-x-2">
            ...
            <div class="category-details">
              ...
              <div v-if="category.name_fa" class="category-name-fa">{{ category.name_fa }}</div>
              <div v-if="category.name_cn" class="category-name-cn">{{ category.name_cn }}</div>
            </div>
          </div>
...
export default {
  ...
  emits: ['toggle-expansion', 'select-category', 'edit-category', 'delete-category'],
  setup(props, { emit }) {
    const categoriesStore = useCategoriesStore()
    const showActionsMenu = ref(false)

    const isExpanded = computed(() => props.expandedNodes.has(props.category.id))
    const isSelected = computed(() => props.selectedCategory?.id === props.category.id)
    const hasChildren = computed(() => props.category.children_count > 0)
    const children = computed(() => categoriesStore.getChildren(props.category.id))
