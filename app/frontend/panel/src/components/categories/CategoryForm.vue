        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          ...
          <div v-if="!isEditing || (isEditing && !category.parent_id)">
            ...
            <p class="mt-1 text-xs text-gray-500">
              Select a parent category to create a subcategory, or leave empty
              for a root category.
            </p>
          </div>
  ...
  setup(props, { emit }) {
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        error.value =
          err.message ||
          `Failed to ${isEditing.value ? "update" : "create"} category`;
      } finally {
        loading.value = false;
      }
    };
