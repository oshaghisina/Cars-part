<template>
  <div class="ai-search-interface">
    ...
    <!-- Query Analysis -->
    <div v-if="queryAnalysis" class="query-analysis mb-6 p-4 bg-blue-50 rounded-lg">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <span class="text-gray-500">({{ Math.round(queryAnalysis.language_confidence * 100) }}% confidence)</span>
        </div>
        <div>
      ...
      <div v-if="queryAnalysis.entities && Object.keys(queryAnalysis.entities).length > 0" class="mt-3">
        <div class="font-medium text-blue-800 mb-1">Extracted Entities:</div>
        <div class="flex flex-wrap gap-2">
          <span v-for="(values, key) in queryAnalysis.entities" :key="key" class="text-xs">
    ...
    <div v-if="searchResults.length > 0" class="search-results">
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex gap-2">
          <select v-model="searchType" @change="performSearch" class="px-3 py-1 border border-gray-300 rounded text-sm">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          <div class="text-sm text-gray-600 mb-2">
            ...
            <div><strong>Vehicle:</strong> {{ result.vehicle_make }} {{ result.vehicle_model }}</div>
            <div><strong>Category:</strong> {{ result.category }}</div>
            <div v-if="result.price"><strong>Price:</strong> ${{ result.price }}</div>
          </div>
          ...
          <div class="flex justify-between items-center">
            ...
            <span :class="result.availability ? 'text-green-600' : 'text-red-600'" class="text-xs font-medium">
    ...
    <!-- No Results -->
    <div v-else-if="hasSearched && !isLoading" class="no-results text-center py-8">
      <div class="text-gray-500 text-lg mb-2">No results found</div>
      <div class="text-gray-400 text-sm">Try adjusting your search terms or using different keywords</div>
    </div>
...
const getSuggestions = async () => {
  try {
    const response = await fetch(`/api/v1/ai/search-suggestions?query=${encodeURIComponent(searchQuery.value)}&limit=5`, {
      headers: {
