<template>
  <div class="bg-white rounded-lg shadow p-6">
    ...
    <!-- Linked Account Info -->
    <div v-if="isLinked && telegramUser" class="mb-6 p-4 bg-green-50 rounded-lg">
      <div class="flex items-center space-x-3 space-x-reverse">
        ...
        <div>
          ...
          <p class="text-xs text-gray-500">ID: {{ telegramUser.telegram_id }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="!isLinked" class="mb-6">
      <div class="mb-4">
        <label for="telegram-id" class="block text-sm font-medium text-gray-700 mb-2">
          شناسه تلگرام شما
        </label>
        <input
          id="telegram-id"
    ...
    <!-- Success Message -->
    <div v-if="successMessage" class="mt-4 bg-green-50 border border-green-200 rounded-md p-3">
...
export default {
  ...
  setup() {
    ...
    const linkAccount = async () => {
      ...
      try {
        const result = await authStore.linkTelegramAccount(parseInt(telegramId.value))
        
        if (result.success) {
          successMessage.value = 'لینک اتصال ارسال شد. لطفاً در تلگرام تأیید کنید.'
          telegramId.value = ''
        } else {
