  ...
  <div class="ai-language-analysis">
    <div class="analysis-header mb-6">
      ...
      <p class="text-gray-600">
        Analyze text for language detection, entity extraction, and search optimization
      </p>
    </div>
    ...
    <div v-if="analysisResult" class="analysis-results">
      ...
      <div class="result-section mb-6 p-4 bg-blue-50 rounded-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <span class="ml-2 px-2 py-1 bg-blue-100 text-blue-800 rounded text-sm font-medium">
              {{ analysisResult.language }}
            </span>
          </div>
          <div>
            <span class="font-medium">Confidence:</span>
            <span class="ml-2">{{ Math.round(analysisResult.language_confidence * 100) }}%</span>
          </div>
          <div>
            <span class="font-medium">Contains Persian:</span>
            <span class="ml-2" :class="analysisResult.contains_persian ? 'text-green-600' : 'text-gray-600'">
              {{ analysisResult.contains_persian ? 'Yes' : 'No' }}
            </span>
          </div>
          <div>
            <span class="font-medium">Contains English:</span>
            <span class="ml-2" :class="analysisResult.contains_english ? 'text-green-600' : 'text-gray-600'">
      ...
      <div class="result-section mb-6 p-4 bg-green-50 rounded-lg">
        ...
        <div class="space-y-2">
          <div>
            ...
            <div class="mt-1 p-2 bg-white border rounded text-sm">{{ analysisResult.original_text }}</div>
          </div>
          <div>
            <span class="font-medium text-sm">Normalized Text:</span>
            <div class="mt-1 p-2 bg-white border rounded text-sm">{{ analysisResult.normalized_text }}</div>
          </div>
        </div>
      </div>

      <!-- Entity Extraction -->
      <div v-if="analysisResult.entities && Object.keys(analysisResult.entities).length > 0" class="result-section mb-6 p-4 bg-purple-50 rounded-lg">
        <h3 class="font-semibold text-purple-800 mb-3">Extracted Entities</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-for="(values, key) in analysisResult.entities" :key="key" class="space-y-1">
            <div class="font-medium text-sm capitalize">{{ key.replace('_', ' ') }}:</div>
      ...
      <!-- Search Variants -->
      <div v-if="analysisResult.search_variants && analysisResult.search_variants.length > 0" class="result-section mb-6 p-4 bg-yellow-50 rounded-lg">
      ...
      <div class="result-section mb-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            ...
            <span class="ml-2" :class="analysisResult.is_mixed ? 'text-orange-600' : 'text-gray-600'">
              {{ analysisResult.is_mixed ? 'Yes' : 'No' }}
            </span>
          </div>
          <div>
            <span class="font-medium">Text Length:</span>
            <span class="ml-2">{{ analysisResult.original_text?.length || 0 }} characters</span>
          </div>
          <div>
            <span class="font-medium">Word Count:</span>
            <span class="ml-2">{{ analysisResult.original_text?.split(' ').length || 0 }} words</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- No Analysis -->
    <div v-else-if="hasAnalyzed && !isLoading" class="no-analysis text-center py-8">
