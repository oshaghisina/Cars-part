  ...
  <div class="ai-chat-page">
    ...
    <div class="page-header">
      <div class="header-content">
        <div class="header-title">
          ...
          <p class="text-gray-600">Get help with system monitoring, troubleshooting, and analysis</p>
        </div>
    ...
    <div v-if="showStatusPanel" class="status-panel">
      <div class="status-header">
        ...
        <button @click="showStatusPanel = false" class="close-button">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="status-content">
        <div v-if="aiStatus" class="status-grid">
          <div class="status-item">
            ...
            <div class="status-value" :class="aiStatus.enabled ? 'text-green-600' : 'text-red-600'">
              {{ aiStatus.enabled ? 'Enabled' : 'Disabled' }}
            </div>
          </div>
          <div class="status-item">
            <div class="status-label">Providers</div>
            <div class="status-value">{{ Object.keys(aiStatus.providers || {}).length }}</div>
          </div>
          <div class="status-item">
            <div class="status-label">Cache Hit Rate</div>
            <div class="status-value">{{ aiStatus.caching?.hit_rate || 0 }}%</div>
          </div>
          <div class="status-item">
            <div class="status-label">Performance</div>
            <div class="status-value">
              {{ aiStatus.performance?.global_metrics?.success_rate || 0 }}% Success
            </div>
          </div>
        </div>
  ...
  setup() {
    ...
    const clearChat = () => {
      if (aiChat.value) {
        ...
        aiChat.value.addMessage('ai', 'Hello! I\'m your AI assistant. How can I help you today?')
      }
    }
