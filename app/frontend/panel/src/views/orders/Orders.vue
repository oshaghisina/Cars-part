<template>
  <div class="space-y-6">
    ...
    <div class="bg-white shadow rounded-lg p-6">
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-gray-600 mt-2">Track and manage customer orders with comprehensive details</p>
        </div>
          ...
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd class="text-lg font-medium text-gray-900">{{ ordersStore.stats.totalOrders }}</dd>
              </dl>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd class="text-lg font-medium text-gray-900">{{ ordersStore.stats.pendingOrders }}</dd>
              </dl>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd class="text-lg font-medium text-gray-900">{{ ordersStore.stats.completedOrders }}</dd>
              </dl>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd class="text-lg font-medium text-gray-900">${{ formatCurrency(ordersStore.stats.totalValue) }}</dd>
              </dl>
            </div>
          </div>
        </div>
    ...
    <div class="bg-white shadow rounded-lg">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else v-for="order in ordersStore.orders" :key="order.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900">{{ getCustomerName(order.lead_id) }}</div>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="text-xs text-gray-500">{{ getTotalQuantity(order.items) }} units</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-medium text-gray-900">${{ formatCurrency(calculateOrderValue(order)) }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
      ...
      <!-- Pagination -->
      <div v-if="ordersStore.pagination.totalPages > 1" class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
        <div class="flex-1 flex justify-between sm:hidden">
          ...
          <button
            @click="goToNextPage"
            :disabled="ordersStore.pagination.page === ordersStore.pagination.totalPages"
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          <div>
            <p class="text-sm text-gray-700">
              Showing
              <span class="font-medium">{{ (ordersStore.pagination.page - 1) * ordersStore.pagination.limit + 1 }}</span>
              to
              <span class="font-medium">{{ Math.min(ordersStore.pagination.page * ordersStore.pagination.limit, ordersStore.pagination.total) }}</span>
              of
              <span class="font-medium">{{ ordersStore.pagination.total }}</span>
              results
            </p>
          </div>
          <div>
    ...
    <OrderDetailModal v-if="showOrderModal" :order="selectedOrder" @close="showOrderModal = false" @updated="onOrderUpdated" />
    
    <!-- Status Update Modal will be added here -->
    <StatusUpdateModal v-if="showStatusModal" :order="selectedOrder" @close="showStatusModal = false" @updated="onOrderUpdated" />
  </div>
</template>
  ...
  setup() {
    ...
    const refreshData = async () => {
      ...
      leadsStore.leads.forEach(lead => {
        customerNames.value[lead.id] = `${lead.first_name || ''} ${lead.last_name || ''}`.trim() || 'Unknown Customer'
      })
    }
