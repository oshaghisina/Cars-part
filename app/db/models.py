...
class VehicleModel(Base):
    ...
    body_type = Column(String(50), nullable=True)  # SUV, Sedan, Hatchback, etc.
    description = Column(Text, nullable=True)
...
class VehicleTrim(Base):
    ...
    fuel_type = Column(String(50), nullable=True)  # Gasoline, Diesel, Hybrid, Electric
...
class PartCategory(Base):
    ...
    level = Column(Integer, default=0)  # Category level (0=root, 1=sub, 2=sub-sub)
    path = Column(String(500), nullable=True)  # Full path like "/Engine/Engine Parts/Filters"
    is_active = Column(Boolean, default=True, nullable=False)
    sort_order = Column(Integer, default=0)
    created_at = Column(DateTime, nullable=False, default=func.now())
    updated_at = Column(DateTime, nullable=False, default=func.now(), onupdate=func.now())
...
class PartSpecification(Base):
    ...
    spec_name = Column(String(100), nullable=False)  # e.g., "Length", "Material", "Weight"
    spec_value = Column(String(200), nullable=False)  # e.g., "120mm", "Steel", "2.5kg"
    spec_unit = Column(String(20), nullable=True)  # e.g., "mm", "kg", "V"
    spec_type = Column(String(20), default="text")  # text, number, boolean, enum
...
class PartImage(Base):
    ...
    image_type = Column(String(20), default="main")  # main, thumbnail, detail, installation
