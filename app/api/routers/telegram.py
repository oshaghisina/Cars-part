...
async def request_telegram_link(
    ...
    try:
        ...
        telegram_user = telegram_service.get_or_create_telegram_user(
            request.telegram_id, telegram_user_info
        )
...
async def verify_telegram_link(
    ...
    try:
        ...
        if not link_token:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid or expired link token"
            )
        ...
        if telegram_user.is_linked:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT, detail="Telegram account is already linked"
            )
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to link Telegram account"
            )
        ...
        logger.info(
            f"Successfully linked Telegram account {telegram_user.telegram_id} "
...
async def telegram_login(
    request: TelegramLoginRequest, http_request: Request, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        if not platform_user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Linked platform user not found"
            )
        ...
        logger.info(
            f"Telegram login successful for user {platform_user.username} ({platform_user.id})"
        )
...
async def unlink_telegram_account(
    ...
    try:
        ...
        # Find Telegram user
        telegram_user = (
            db.query(TelegramUser).filter(TelegramUser.telegram_id == request.telegram_id).first()
        )

        if not telegram_user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Telegram account not found"
            )
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to unlink Telegram account"
            )

        logger.info(
            f"Successfully unlinked Telegram account {request.telegram_id} "
...
async def verify_telegram_deep_link(
    request: TelegramDeepLinkVerifyRequest, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        if not deep_link:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid or expired deep link"
            )
...
async def telegram_webhook(request: TelegramWebhookRequest, db: Session = Depends(get_db)):
    """
            ...
            if chat_id and text:
                ...
                if text.startswith("/start"):
                    ...
                    if " " in text:
                        ...
                        if deep_link:
                            ...
                            if deep_link.action == "login":
                                await telegram_service.send_telegram_message(
                                    chat_id, "🔐 Login successful! You can now access your account."
                                )
                            elif deep_link.action == "link_account":
                                await telegram_service.send_telegram_message(
                                    chat_id,
                                    "🔗 Account linking initiated! "
                                    "Please complete the process on the web interface.",
                                )
                            deep_link.mark_used()
                            db.commit()
                            processed = True
                        ...
                        await telegram_service.send_telegram_message(
                            chat_id, "👋 Welcome! Use /help to see available commands."
                        )
                        processed = True
...
Use /link to link your Telegram account to your platform account.
        ...
        return TelegramWebhookResponse(success=True, message=message, processed=processed)

    except Exception as e:
        logger.error(f"Error processing Telegram webhook: {e}")
        return TelegramWebhookResponse(
            success=False, message="Error processing webhook", processed=False, code="WEBHOOK_ERROR"
        )
