...
async def list_categories(
    ...
    children_counts = (
        ...
        .filter(PartCategory.parent_id.in_(category_ids))
        .group_by(PartCategory.parent_id)
        .all()
    )
    ...
    # Get parts counts
    parts_counts = (
        db.query(Part.category_id, func.count(Part.id).label("count"))
        .filter(Part.category_id.in_(category_ids))
    ...
    return [
        PartCategoryResponse(
            ...
            children_count=children_count_map.get(
                category.id, 0
