    ...
    try:
        with Image.open(original_path) as img:
            ...
            for size_name, dimensions in IMAGE_SIZES.items():
                ...
                # Generate filename
                thumb_filename = f"part_{part_id}_{image_type}_{size_name}_{uuid.uuid4().hex[:8]}.jpg"
...
async def get_image_stats(
    ...
    active_images = db.query(PartImage).filter(PartImage.is_active == True).count()
    ...
    ).filter(PartImage.is_active == True).group_by(PartImage.image_type).all()
