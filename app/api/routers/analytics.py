...
async def get_dashboard_metrics(
    ...
    try:
        ...
        pending_orders = db.query(Order).filter(
            and_(Order.status == 'pending', date_filter)
        ).count()
...
async def get_sales_analytics(
    ...
    try:
        ...
        ).join(Price, Part.id == Price.part_id).filter(
            and_(
...
async def get_inventory_analytics(
    ...
    try:
        ...
        ).join(Part, Part.category_id == PartCategory.id).join(
            Price, Part.id == Price.part_id
        ).group_by(
            PartCategory.name
        ).all()
...
async def get_performance_metrics(
    ...
    try:
        ...
        bot_metrics = {
            ...
            "active_sessions": db.query(WizardSession).filter(
                func.date(WizardSession.created_at) == date.today()
            ).count(),
