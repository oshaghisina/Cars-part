...
class OTPService:
    ...
    def __init__(self, db: Session):
        ...
        self.rate_limits = {
            ...
            "login_attempt": {"count": 10, "window_minutes": 15},  # 10 login attempts per 15 minutes
        }
    ...
    def check_rate_limit(self, identifier: str, identifier_type: str, action: str) -> Tuple[bool, str]:
        """
        ...
        if rate_limit.count >= limit_config["count"]:
            return False, f"Rate limit exceeded. Max {limit_config['count']} {action} per {limit_config['window_minutes']} minutes"
    ...
    async def request_otp(self, phone_number: str, code_type: str = "login", user_id: Optional[int] = None) -> Dict:
        """
        ...
        try:
            ...
            active_otp = self.db.query(OTPCode).filter(
                and_(
            ...
            if active_otp:
                ...
                if sms_result["success"]:
                    ...
                    return {"success": False, "message": f"SMS sending failed: {sms_result['message']}", "code": "SMS_FAILED"}
            ...
            if sms_result["success"]:
                ...
                return {"success": True, "message": "OTP code sent", "expires_in": self.code_expiry_minutes * 60}
            else:
    ...
    async def verify_otp(self, phone_number: str, code: str, code_type: str = "login") -> Dict:
        ...
        try:
            ...
            if otp_record.is_exhausted():
                ...
                return {"success": False, "message": "OTP code has exceeded maximum attempts", "code": "OTP_EXHAUSTED"}
            ...
            if self.verify_otp_code(code, otp_record.code_hash):
                ...
                if otp_record.is_exhausted():
                    ...
                    logger.warning(f"OTP verification failed for {phone_number}, {remaining_attempts} attempts remaining")
                    return {"success": False, "message": f"Invalid OTP code. {remaining_attempts} attempts remaining", "code": "INVALID_OTP"}
    ...
    async def _send_otp_sms(self, phone_number: str, code: str, code_type: str) -> Dict:
        ...
        try:
            ...
            if code_type == "login":
                message = f"Your login code is: {code}. Valid for 5 minutes. Do not share this code."
                ...
                message = f"Your password reset code is: {code}. Valid for 5 minutes. Do not share this code."
            else:
                message = f"Your code is: {code}. Valid for 5 minutes. Do not share this code."
            ...
            if result.success:
                ...
                return {
                    "success": False,
                    "message": result.message or "SMS sending failed"
                }
    ...
    def get_otp_stats(self, phone_number: str) -> Dict:
        ...
        try:
            # Count active OTPs
            active_count = self.db.query(OTPCode).filter(
                and_(
                    OTPCode.phone_number == phone_number,
                    OTPCode.is_used == False,
                    OTPCode.expires_at > func.now()
            ...
            total_24h = self.db.query(OTPCode).filter(
                and_(
                    OTPCode.phone_number == phone_number,
                    OTPCode.created_at >= cutoff_24h
                )
            ).count()
