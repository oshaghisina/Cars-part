...
from app.services.ai_language_processor import Language, LanguageProcessor
...
class AIQueryProcessor:
    ...
    def _load_clarification_templates(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "model": ["کدام مدل خودرو؟", "برای کدام مدل؟", "Which vehicle model?"],
            "position": ["جلو یا عقب؟", "چپ یا راست؟", "Front or rear? Left or right?"],
            "price_range": ["چه محدوده قیمتی مد نظرتان است؟", "What's your budget range?"],
            "urgency": ["فوری نیاز دارید؟", "Do you need it urgently?"],
        }
    ...
    def _check_clarification_needed(
        ...
        if intent == QueryIntent.PRICE_INQUIRY and not any(
            word in query.lower() for word in ["قیمت", "price", "هزینه", "cost"]
        ):
    ...
    def generate_response_suggestions(self, analysis: QueryAnalysis) -> List[str]:
        ...
        if analysis.intent == QueryIntent.PRICE_INQUIRY:
            suggestions.extend(["قیمت‌های مختلف بر اساس برند", "مقایسه قیمت‌ها", "بهترین قیمت‌ها"])
        elif analysis.intent == QueryIntent.AVAILABILITY_CHECK:
            suggestions.extend(["موجودی فعلی", "زمان تحویل", "رزرو کردن"])
        elif analysis.intent == QueryIntent.RECOMMENDATION:
            suggestions.extend(["توصیه‌های متخصصان", "محصولات محبوب", "بررسی‌های مشتریان"])
        elif analysis.intent == QueryIntent.MAINTENANCE:
            suggestions.extend(["برنامه تعمیرات", "قسمت‌های مورد نیاز", "هزینه تعمیرات"])
