...
class PartsServiceEnhanced:
    ...
    def update_part_with_version(
        ...
        try:
            ...
            part = self.db.query(Part).filter(
                and_(
                    Part.id == part_id,
                    Part.current_version == expected_version
                )
            ...
            try:
                try:
                    ...
                    loop.create_task(event_bus.emit('part_updated', {
                        'part_id': part_id,
                        'changes': changes,
                    ...
                    asyncio.run(event_bus.emit('part_updated', {
                        'part_id': part_id,
                        'changes': changes,
                        'updated_by': updated_by,
                        'new_version': part.current_version,
    ...
    def get_part_history(self, part_id: int, limit: int = 10) -> List[PartVersion]:
        ...
        try:
            history = self.db.query(PartVersion).filter(
                PartVersion.part_id == part_id
            ).order_by(PartVersion.created_at.desc()).limit(limit).all()
    ...
    def get_part_by_version(self, part_id: int, version: int) -> Optional[Part]:
        ...
        try:
            ...
            version_entry = self.db.query(PartVersion).filter(
                and_(
                    PartVersion.part_id == part_id,
                    PartVersion.version == version
                )
            ).first()
    ...
    def rollback_part_to_version(self, part_id: int, target_version: int, rolled_back_by: str = "system") -> bool:
        ...
        try:
            ...
            for field in ['part_name', 'brand_oem', 'vehicle_make', 'vehicle_model', 
                         'vehicle_trim', 'model_year_from', 'model_year_to', 
                         'engine_code', 'position', 'category', 'subcategory', 
                         'oem_code', 'alt_codes', 'dimensions_specs', 
                         'compatibility_notes', 'unit', 'pack_size', 'status']:
...
# Import asyncio for event emission
import asyncio
