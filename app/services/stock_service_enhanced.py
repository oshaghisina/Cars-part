...
class StockServiceEnhanced:
    ...
    def set_part_stock_with_version(
        ...
        try:
            ...
            if not stock:
                ...
                existing_stock = self.db.query(StockLevel).filter(
                    StockLevel.part_id == part_id
                ).first()
            ...
            try:
                ...
                try:
                    ...
                    loop.create_task(event_bus.emit('stock_updated', {
                        'part_id': part_id,
                    ...
                    asyncio.run(event_bus.emit('stock_updated', {
                        'part_id': part_id,
                        'stock_changes': changes,
                        'updated_by': updated_by,
                        'new_version': stock.version
    ...
    def get_stock_history(self, part_id: int, limit: int = 10) -> List[StockVersion]:
        ...
        try:
            ...
            # Get version history
            history = self.db.query(StockVersion).filter(
                StockVersion.stock_id == stock.id
            ).order_by(StockVersion.created_at.desc()).limit(limit).all()
    ...
    def rollback_stock_to_version(self, part_id: int, target_version: int, rolled_back_by: str = "system") -> bool:
    ...
    def get_stock_statistics(self) -> Dict:
        ...
        try:
            ...
            # Out of stock parts
            out_of_stock = self.db.query(StockLevel).filter(
            ...
            recently_updated = self.db.query(StockLevel).filter(
                StockLevel.updated_at >= recent_cutoff
            ).count()
            ...
            return {
                ...
                'out_of_stock_percentage': (out_of_stock / total_parts * 100) if total_parts > 0 else 0
            }
        except Exception as e:
