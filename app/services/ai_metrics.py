...
class AIMetricsCollector:
    ...
    def record_request(
        ...
        **tags
        ...
        # Add metric points
        self._add_metric_point(f"ai_request_duration", duration_ms, {
            "provider": provider,
            "task_type": task_type,
            "success": str(success)
        ...
        if cost is not None:
            self._add_metric_point(f"ai_request_cost", cost, {
                "provider": provider,
                "task_type": task_type
            }, MetricType.COUNTER)
    ...
    def record_provider_health(self, provider: str, status: str, response_time_ms: Optional[float] = None):
        ...
        self._add_metric_point(f"ai_provider_health", 1 if status == "healthy" else 0, {
            "provider": provider,
            "status": status
        }, MetricType.GAUGE)
        
        if response_time_ms is not None:
            self._add_metric_point(f"ai_provider_response_time", response_time_ms, {
                "provider": provider
            }, MetricType.HISTOGRAM)

    def record_cache_metrics(self, operation: str, hit: bool, size: int):
        """Record cache-related metrics."""
        self._add_metric_point(f"ai_cache_operation", 1 if hit else 0, {
            "operation": operation,
            "hit": str(hit)
        }, MetricType.COUNTER)
        
        self._add_metric_point(f"ai_cache_size", size, {
            "operation": operation
        }, MetricType.GAUGE)

    def record_rate_limit(self, provider: str, limit_type: str, rate: float):
        """Record rate limiting metrics."""
        self._add_metric_point(f"ai_rate_limit", rate, {
            "provider": provider,
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        for provider, data in summary["providers"].items():
            if data["success_rate"] < 0.9:
                health_status["issues"].append(f"Provider '{provider}' has low success rate: {data['success_rate']:.2%}")
    ...
    def _export_prometheus_format(self) -> str:
        ...
        for key, metrics in self.metrics.items():
            ...
            # Success count
            lines.append(f'ai_successes_total{{provider="{provider}", task_type="{task_type}"}} {metrics.success_count}')
            
            # Failure count
            lines.append(f'ai_failures_total{{provider="{provider}", task_type="{task_type}"}} {metrics.failure_count}')
            
            # Duration metrics
            lines.append(f'ai_duration_seconds{{provider="{provider}", task_type="{task_type}", quantile="0.5"}} {metrics.p50_duration_ms / 1000}')
            lines.append(f'ai_duration_seconds{{provider="{provider}", task_type="{task_type}", quantile="0.95"}} {metrics.p95_duration_ms / 1000}')
            lines.append(f'ai_duration_seconds{{provider="{provider}", task_type="{task_type}", quantile="0.99"}} {metrics.p99_duration_ms / 1000}')
            
            # Cost metrics
            for cost_key, cost in metrics.cost_tracking.items():
                lines.append(f'ai_cost_total{{provider="{provider}", task_type="{task_type}", cost_type="{cost_key}"}} {cost}')
