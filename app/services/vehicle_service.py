...
class VehicleService:
    ...
    def get_brands(
        ...
        return query.order_by(VehicleBrand.sort_order,
    ...
    def get_models(
        ...
        return query.order_by(VehicleModel.sort_order,
                              VehicleModel.name).offset(skip).limit(limit).all()
    ...
    def get_models_by_brand(self, brand_id: int) -> List[VehicleModel]:
        ...
        ).order_by(VehicleModel.sort_order, VehicleModel.name).all()

    def get_model_by_id(self, model_id: int) -> Optional[VehicleModel]:
        """Get model by ID."""
        return self.db.query(VehicleModel).filter(
            VehicleModel.id == model_id).first()
    ...
    def get_trims(
        ...
        if brand_id:
            query = query.join(VehicleModel).filter(
                VehicleModel.brand_id == brand_id)
        ...
        return query.order_by(VehicleTrim.sort_order,
                              VehicleTrim.name).offset(skip).limit(limit).all()

    def get_trims_by_model(self, model_id: int) -> List[VehicleTrim]:
        """Get all trims for a specific model."""
        return self.db.query(VehicleTrim).filter(
            VehicleTrim.model_id == model_id,
            VehicleTrim.is_active
        ).order_by(VehicleTrim.sort_order, VehicleTrim.name).all()

    def get_trim_by_id(self, trim_id: int) -> Optional[VehicleTrim]:
        """Get trim by ID."""
        return self.db.query(VehicleTrim).filter(
            VehicleTrim.id == trim_id).first()
    ...
    def get_brand_model_trim_tree(self) -> List[Dict[str, Any]]:
        ...
        for brand in brands:
            ...
            models = self.get_models_by_brand(
                int(brand.id))  # type: ignore[arg-type]
            for model in models:
                model_data = {
                    "id": model.id,
                    "name": model.name,
                ...
                trims = self.get_trims_by_model(
                    int(model.id))  # type: ignore[arg-type]
                for trim in trims:
                    trim_data = {
                        "id": trim.id,
                        "name": trim.name,
    ...
    def search_vehicles(
        ...
        # Search models
        models = self.db.query(VehicleModel).join(VehicleBrand).filter(
            VehicleModel.is_active,
            VehicleBrand.is_active,
            or_(
        ...
        # Search trims
        trims = self.db.query(VehicleTrim).join(VehicleModel).join(VehicleBrand).filter(
            VehicleTrim.is_active,
            VehicleModel.is_active,
            VehicleBrand.is_active,
            or_(
