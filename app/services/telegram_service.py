...
class TelegramService:
    ...
    def verify_link_token(self, token: str, action: str = "link_account") -> Optional[TelegramLinkToken]:
        """Verify and retrieve link token."""
        # Find token by hash
        for link_token in self.db.query(TelegramLinkToken).filter(
            and_(
                TelegramLinkToken.action == action,
                TelegramLinkToken.is_used == False,
    ...
    def verify_deep_link(self, link_id: str) -> Optional[TelegramDeepLink]:
        ...
        deep_link = self.db.query(TelegramDeepLink).filter(
            and_(
                TelegramDeepLink.link_id == link_id,
                TelegramDeepLink.is_used == False,
    ...
    def link_telegram_account(self, telegram_user: TelegramUser, user_id: int) -> bool:
        ...
        try:
            ...
            if telegram_user.is_linked and telegram_user.user_id:
                logger.warning(f"Telegram user {telegram_user.telegram_id} already linked to user {telegram_user.user_id}")
                return False
            
            # Check if platform user already has Telegram account
            existing_telegram = self.db.query(TelegramUser).filter(
                and_(
            ...
            logger.info(f"Successfully linked Telegram user {telegram_user.telegram_id} to platform user {user_id}")
            return True
    ...
    def create_bot_session(
        ...
        session = self.db.query(TelegramBotSession).filter(
            and_(
    ...
    def get_bot_session(self, telegram_user: TelegramUser) -> Optional[TelegramBotSession]:
        ...
        return self.db.query(TelegramBotSession).filter(
            and_(
                TelegramBotSession.telegram_user_id == telegram_user.id,
                TelegramBotSession.expires_at > func.now()
            )
    ...
    def generate_telegram_login_url(self, telegram_user: TelegramUser, action: str = "login") -> str:
    ...
    async def send_telegram_message(self, telegram_id: int, message: str, parse_mode: str = "HTML") -> bool:
    ...
    def cleanup_expired_tokens(self):
        ...
        try:
            ...
            expired_tokens = self.db.query(TelegramLinkToken).filter(
                TelegramLinkToken.expires_at < func.now()
            ).all()
            ...
            expired_links = self.db.query(TelegramDeepLink).filter(
                TelegramDeepLink.expires_at < func.now()
            ).all()
            ...
            expired_sessions = self.db.query(TelegramBotSession).filter(
                TelegramBotSession.expires_at < func.now()
            ).all()
            ...
            if expired_tokens or expired_links or expired_sessions:
                self.db.commit()
                logger.info(f"Cleaned up {len(expired_tokens)} tokens, {len(expired_links)} links, {len(expired_sessions)} sessions")
    ...
    def get_telegram_stats(self) -> Dict:
        ...
        try:
            ...
            linked_users = self.db.query(TelegramUser).filter(TelegramUser.is_linked == True).count()
            active_sessions = self.db.query(TelegramBotSession).filter(
                TelegramBotSession.expires_at > func.now()
            ).count()
            ...
            return {
                ...
                "link_rate": (linked_users / total_telegram_users * 100) if total_telegram_users > 0 else 0
            }
