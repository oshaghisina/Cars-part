...
class LanguageProcessor:
    ...
    def _load_persian_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "persian_car_brands": re.compile(r'\b(چری|جک|بریلیانس|بید|جیلی|گریت وال|ام جی|هیوندای|کیا|نیسان|تویوتا|هوندا|میتسوبیشی|سوزوکی|مزدا|سوبارو|ایسوزو|داچیا|رنو|پژو|سیتروئن|فیات|آلفا رومئو|لانچیا|مازراتی|فراری|لامبورگینی|مکلارن|آستون مارتین|بنتلی|رولز رویس|بامو|مرسدس|آئودی|پورشه|فولکس واگن|اسکودا|سیات|آلفا رومئو)\b', re.IGNORECASE),
            "persian_part_types": re.compile(r'\b(لنت|فیلتر|موتور|تعلیق|گیربکس|کلاچ|دifferential|کمک فنر|فنر|کمربند|تسمه|شمع|سیم|کابل|باطری|آلترناتور|استارت|پمپ|رادیاتور|ترموستات|سنسور|شیر|شلنگ|لوله|میل|شفت|یاتاقان|بوش|واشر|پیچ|مهره|پولک|رینگ|تایر|لاستیک|چرخ|فرمان|دستی|اتومات|دستی|کامپیوتر|ای سی یو|مپ|سنسور|شیر|شلنگ|لوله|میل|شفت|یاتاقان|بوش|واشر|پیچ|مهره|پولک|رینگ|تایر|لاستیک|چرخ|فرمان|دستی|اتومات|دستی|کامپیوتر|ای سی یو|مپ)\b', re.IGNORECASE),
        }
    ...
    def _load_english_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "english_car_brands": re.compile(r'\b(Chery|JAC|Brilliance|BYD|Geely|Great Wall|MG|Hyundai|Kia|Nissan|Toyota|Honda|Mitsubishi|Suzuki|Mazda|Subaru|Isuzu|Dacia|Renault|Peugeot|Citroen|Fiat|Alfa Romeo|Lancia|Maserati|Ferrari|Lamborghini|McLaren|Aston Martin|Bentley|Rolls Royce|BMW|Mercedes|Audi|Porsche|Volkswagen|Skoda|Seat)\b', re.IGNORECASE),
            "english_part_types": re.compile(r'\b(brake|filter|engine|suspension|transmission|clutch|differential|shock|spring|belt|spark|wire|cable|battery|alternator|starter|pump|radiator|thermostat|sensor|valve|hose|pipe|shaft|bearing|bush|gasket|screw|nut|washer|ring|tire|wheel|steering|manual|automatic|computer|ECU|MAP)\b', re.IGNORECASE),
        }
    ...
    def translate_terms(self, text: str, target_language: Language) -> str:
        ...
        for source, target in self.car_terms_mapping["brands"].items():
            if target_language == Language.PERSIAN and source.isascii():
                translated_text = re.sub(r'\b' + re.escape(source) + r'\b', target, translated_text, flags=re.IGNORECASE)
