...
class SettingsService:
    ...
    def get_settings(self, keys: Optional[list] = None) -> Dict[str, str]:
        ...
        settings = query.all()
        return {setting.key: setting.value for setting in settings}  # type: ignore[misc]
    ...
    def get_system_settings(self) -> Dict[str, Any]:
        ...
        # Integer settings
        int_keys = ["BULK_LIMIT_DEFAULT", "MAX_UPLOAD_SIZE", "SESSION_TIMEOUT", "BACKUP_INTERVAL_HOURS"]
        for key in int_keys:
            if key in raw_settings:
                try:
