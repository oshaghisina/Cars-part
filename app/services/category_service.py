...
class CategoryService:
    ...
    def get_categories(
        ...
        return query.options(selectinload(PartCategory.children), selectinload(PartCategory.parts)).order_by(PartCategory.sort_order, PartCategory.name).offset(skip).limit(limit).all()
    ...
    def get_category_path(self, category_id: int) -> List[PartCategory]:  # type: ignore[return]
        ...
        while category:
            ...
            if category.parent_id:  # type: ignore[comparison-overlap]
                category = self.get_category_by_id(category.parent_id)  # type: ignore[arg-type]
            else:
                break
        
        return path  # type: ignore[return-value]
    
    def get_category_tree(self, include_inactive: bool = False) -> List[PartCategory]:
        """Get the complete category tree structure."""
        query = self.db.query(PartCategory)
        
        if not include_inactive:
            query = query.filter(PartCategory.is_active == True)
        ...
        for category in all_categories:
            if category.parent_id is None:  # type: ignore[comparison-overlap]
                ...
                parent = category_dict.get(category.parent_id)  # type: ignore[arg-type]
                if parent:
    ...
    def create_category(self, category_data: Dict[str, Any]) -> Optional[PartCategory]:
        ...
        try:
            ...
            if parent_id:
                ...
                if parent:
                    ...
                    path = f"{parent.path}/{category_data['name']}" if parent.path else f"/{category_data['name']}"  # type: ignore[comparison-overlap]
                else:
            ...
            if 'parent_id' in category_data:
                ...
                if new_parent_id != category.parent_id:
                    ...
                    if new_parent_id:
                        ...
                        if parent:
                            category.level = parent.level + 1  # type: ignore[assignment]  # type: ignore[assignment]
                            category.path = f"{parent.path}/{category.name}" if parent.path else f"/{category.name}"  # type: ignore[assignment]  # type: ignore[assignment]
                        else:
                            raise ValueError("Invalid parent_id")
                    else:
                        category.level = 0  # type: ignore[assignment]
                        category.path = f"/{category.name}"  # type: ignore[assignment]  # type: ignore[assignment]
    ...
    def delete_category(self, category_id: int) -> bool:
        ...
        try:
            ...
            self._soft_delete_category_and_children(category_id)  # type: ignore[arg-type]
        ...
        try:
            ...
            if new_parent_id is not None and new_parent_id != category.parent_id:
                ...
                if new_parent_id != 0:  # 0 means root level
                    ...
                    if not parent:
                        return False
                    category.parent_id = new_parent_id  # type: ignore[assignment]
                    category.level = parent.level + 1  # type: ignore[assignment]
                    category.path = f"{parent.path}/{category.name}" if parent.path else f"/{category.name}"  # type: ignore[assignment]
                else:
                    category.parent_id = None  # type: ignore[assignment]
                    category.level = 0  # type: ignore[assignment]
                    category.path = f"/{category.name}"  # type: ignore[assignment]
            ...
            if new_sort_order is not None:
                category.sort_order = new_sort_order  # type: ignore[assignment]
    ...
    def _update_descendant_paths(self, category_id: int, parent_path: str):  # type: ignore[arg-type]
        """Update paths for all descendants of a category."""
        children = self.get_category_children(category_id)
        for child in children:
            child.path = f"{parent_path}/{child.name}"  # type: ignore[assignment]
            self._update_descendant_paths(child.id, child.path)  # type: ignore[arg-type]
    ...
    def _soft_delete_category_and_children(self, category_id: int):
        ...
        if category:
            ...
            children = self.get_category_children(category_id)
            for child in children:
                self._soft_delete_category_and_children(child.id)  # type: ignore[arg-type]
    ...
    def get_category_stats(self) -> Dict[str, Any]:
        ...
        active_categories = self.db.query(PartCategory).filter(PartCategory.is_active == True).count()
