...
class UserService:
    ...
    async def create_user(self, user_data: UserCreate) -> User:
        ...
        if existing_user:
            if existing_user.username == user_data.username:  # type: ignore[comparison-overlap]
                raise ValueError("Username already exists")
            if existing_user.email == user_data.email:  # type: ignore[comparison-overlap]
    ...
    async def update_user(self, user_id: int, user_data: UserUpdate) -> Optional[User]:
        ...
        if user_data.preferences is not None:
            user.preferences = user_data.preferences  # type: ignore[assignment]
    ...
    async def authenticate_user(self, username_or_email: str, password: str, 
                              ip_address: Optional[str] = None, user_agent: Optional[str] = None) -> Optional[User]:
    ...
    async def create_session(self, user: User, ip_address: Optional[str] = None, 
                           user_agent: Optional[str] = None, expires_hours: int = 24):
    ...
    # Note: Session and activity logging methods removed - models not implemented yet
