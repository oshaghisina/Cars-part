            ...
            async with self._lock:
                ...
                if len(self.pool) < self.max_size:
                    try:
                        ...
                        logger.debug(f"Created and acquired {self.resource_type} resource: {resource.get('id', 'unknown')}")
                        return resource
                    except Exception as e:
        ...
        async with self._lock:
            if resource in self.in_use:
                ...
                if mark_unhealthy:
                    ...
                    logger.warning(f"Marked {self.resource_type} resource as unhealthy: {resource.get('id', 'unknown')}")
                else:
                    self.available.add(resource)
    ...
    async def _cleanup_resources(self):
        ...
        async with self._lock:
            ...
            for resource in list(self.available):
                if (len(self.pool) > self.min_size and 
                    resource in self.last_used and
                    current_time - self.last_used[resource] > timedelta(seconds=self.idle_timeout)):
                    await self._destroy_resource(resource)
...
class ResourceLimiter:
    ...
    def get_usage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'utilization': {
                'concurrent_requests': (self.current_usage['concurrent_requests'] / self.limits['max_concurrent_requests']) * 100,
