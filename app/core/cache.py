...
class CacheService:
    ...
    def set_part_list(self, cache_key: str, data: List[Dict[str, Any]], ttl: int = None) -> bool:
        ...
        try:
            ...
            # Add timestamp for freshness checking
            cache_data = {"_cached_at": time.time(), "data": data}
    ...
    def invalidate_part(self, part_id: int) -> bool:
        ...
        try:
            ...
            self._invalidate_pattern("part_list:*")
            self._invalidate_pattern("search_results:*")

            # Pub/Sub notification for distributed invalidation
            self.redis_client.publish(
                "cache_invalidation",
                json.dumps({"type": "part_updated", "part_id": part_id, "timestamp": time.time()}),
            )
    ...
    def invalidate_stock(self, part_id: int) -> bool:
        ...
        try:
            ...
            self._invalidate_pattern("stock_list:*")

            # Pub/Sub notification
            self.redis_client.publish(
                "cache_invalidation",
                json.dumps({"type": "stock_updated", "part_id": part_id, "timestamp": time.time()}),
            )
