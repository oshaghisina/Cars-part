...
# API base URL
API_BASE = f"http://localhost:8001/api/v1"
...
class WizardBotService:
    ...
    def get_parts(self, brand: str | None = None, model: str | None = None, category: str | None = None) -> List[Dict[str, Any]]:
        """Get available parts."""
        try:
            params = {}
            if brand:
...
async def safe_edit_message(callback: CallbackQuery, text: str, reply_markup: InlineKeyboardMarkup | None = None, parse_mode: str | None = None):
    ...
    try:
        ...
        if hasattr(callback.message, 'edit_text'):
            ...
            if edit_method and callable(edit_method):
                ...
                await edit_method(text, reply_markup=reply_markup, parse_mode=parse_mode)  # type: ignore
        else:
...
async def handle_category_selection(callback: CallbackQuery, state: FSMContext):
    ...
    if not parts:
        await safe_edit_message(
        callback,
            f"❌ برای {vehicle_data.get('brand')} {vehicle_data.get('model')} در دسته‌بندی {category} هیچ قطعه‌ای یافت نشد.",
            reply_markup=None
        )
        return
